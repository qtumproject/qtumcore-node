...
/**
 * Will remove a service from qtumcore-node.json
...
/**
 * Will remove the Node.js service and from the qtumcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The qtumcore-node configuration directory
...
function remove(options, done) {
  ...
  var bitcoreConfigPath = path.resolve(configPath, 'qtumcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bitcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a qtumcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to qtumcore-node.json
