...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for qtumd is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and qtumcore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed qtumcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './qtumcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in qtumcore-node services directory
    ...
    if (servicePackage.qtumcoreNode) {
      serviceModule = service.name + '/' + servicePackage.qtumcoreNode;
    }
