...
Bitcoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    ...
    // self.getAddressBalance(address, {}, function(err, data) {
    //
    //     if (err) {
    //         return false
    //     }
    ...
    //                 totalSpent: data.received - data.balance,
    //                 balance: data.balance
    //             });
    //         }
    //     }
...
Bitcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  ...
  function updateWithMempool(confirmedUtxos, mempoolDeltas) {
    ...
    for (var i = 0; i < mempoolDeltas.length; i++) {
      ...
      console.log('confirmedUtxos', confirmedUtxos);
      console.log('confirmedUtxos', mempoolUnspentOutputs);
    ...
    //     return (confirmedUtxosHash[utxo.txid + '_' + utxo.outputIndex] && _.isUndefined(utxo.height)) ? false : true;
    // });
